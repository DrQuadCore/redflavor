...
#endif
{
  ...
  for(int i = 0; i < size-offset_for_rx; i=i+2) 
    if(*(uint16_t*)&h_mem[i] != 0) 
      dirty_cnt++;
#endif
  printf("[%s][%d] dirty_cnt:[%d]\n", __FUNCTION__ , __LINE__, 2*dirty_cnt);
...
#else
__global__ void doorbell_test(void * io_addr, void * desc, uint32_t curr, int* g_mem, int size)
#endif
{
  if(desc == 0) {
	...
	printf("[%s][%d]---START------doorbell_test-------make_pkt------------DEVICE.\n", __FUNCTION__, __LINE__);
	...
	printf("[%s][%d]SIZE:%d.\n\b", __FUNCTION__, __LINE__, size);

	for(int i=0; i< 512; i++) { // making 100 arpreq packets
    memcpy(g_mem+4096*i/4, pktBuf, 60);
	}
// For checking "Made pkt in GDDR"
#if 0
	int count = 0;
	for(int j = 0; j < 4096*512; j++) {
		if(*((uint16_t*)&g_mem[j]) == 0x0608 || *((uint16_t*)&g_mem[j]) == 0x0806 ){
...
#endif
	printf("[%s][%d]-----END------doorbell_test-------make_pkt------------DEVICE.\n\b", __FUNCTION__, __LINE__);
	...
	printf("[%s][%d] linebyline1 .\n", __FUNCTION__, __LINE__);
	int num_packets = 500;
	int index = curr;
	for(int i=0; i<curr; i++) {
    //printf("[%s][%d] %d, txdesc: %p.\n", __FUNCTION__, __LINE__, i, tx_desc);
		tx_desc++;
  }
	printf("[%s][%d] linebyline2 .\n", __FUNCTION__, __LINE__);
...
#error "use_gpu"
  ...
  if(cudaSuccess != cudaHostGetDevicePointer((void**)&tx_desc, (void*)desc, 0)) {
    if(tx_desc == 0) {
      printf("[%s][%d]cudaHostGetDevicePointer fails. desc==NULL.\n", __FUNCTION__, __LINE__);
      // second try
      cudaError_t type = cudaHostGetDevicePointer((void**)&tx_desc, (void*)desc, 0);
      if(type != cudaSuccess) {
        printf("%u\n", type);
        printf("%u\n", cudaErrorInvalidDevice);
        printf("%u\n", cudaErrorSetOnActiveProcess);
        if(type == cudaErrorInvalidDevice)
          printf("cudaErrorInvalidDevice\n");
        if(type == cudaErrorSetOnActiveProcess)
  ...
  doorbell_test<<< 1,1 >>>(dBAR, tx_desc, curr_tx_index_q_zero, g_mem, size);
