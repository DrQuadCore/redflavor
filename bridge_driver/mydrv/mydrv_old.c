...
static int mydrv_test(my_info_t *info, void __user *_params)
{
    ...
    //////////////////////////////////////////
#if 0 // HONESTCHOI added below
    my_err("[%s][%s][%d] nvidia_p2p_dma_map_pages_auto will be called.\n", __FILE__ , __FUNCTION__ , __LINE__);
    if(mr->page_table->gpu_uuid == NULL) 
      my_err("[%s][%s][%d] mr->page_table->gpu_uuid == NULL\n", __FILE__ , __FUNCTION__ , __LINE__);
    nvidia_p2p_dma_mapping_t** dma_map = kmalloc(sizeof(nvidia_p2p_dma_mapping_t**), GFP_KERNEL);
    int ret1 = nvidia_p2p_dma_map_pages_auto(page_table, dma_map);

    my_info("BEFORE: %d\n", ret1); 
    int* tmp2 = (int*)*(dma_map[0]->dma_addresses); 
    my_info("AFTER1: 0x%016llx\t\n", (page_table->pages[0])->physical_address );
    my_info("AFTER1: 0x%016llx\t\n", tmp2);
    my_info("AFTER1: %d\n", tmp2[0] );
    //////////////
    tmp = (uint64_t*)dma_map[0]->dma_addresses;
    //my_info("AFTER1: 0x%016llx\t\n", (mr->page_table->pages[0])->physical_address );
    my_info("AFTER2: 0x%016llx\t%d\n", (mr->page_table->pages[0])->physical_address, (uint64_t)*tmp );
    //my_info("AFTER3: 0x%016llx\t\n", (mr->page_table->pages[0])->physical_address );
    ////////////////////////////////////////
#endif
    //tmp = kmalloc(sizeof(uint64_t), GFP_KERNEL);
    //*tmp = 1;
    //memcpy( (void*)(page_table->pages[0])->physical_address, tmp, sizeof(uint64_t));
    //iowrite8(0x01, 
    //nvidia_p2p_dma_mapping_t** dma_map = kmalloc(sizeof(nvidia_p2p_dma_mapping_t**), GFP_KERNEL);
    //int ret1 = nvidia_p2p_dma_map_pages_auto(page_table, dma_map);
    //tmp = kmap((struct page*)((void*)(page_table->pages[0])));
    //*tmp = 1;
